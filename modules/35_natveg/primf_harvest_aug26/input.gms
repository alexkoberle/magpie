...
$setglobal c35_ad_policy  npiweak
$setglobal c35_aolc_policy  npiweak
...
s35_forest_damage Damage simulation in forests (0=none 1=shifting agriculture 2= Damage from shifting agriculture is faded out by c35_forest_damage_end 4= f35_forest_shock scenario) / 0 /
...
s35_hvarea_clearcut annual primforest clear-cut rate for s35_hvarea equals 1 (percent per year) / 0 /
s35_clearcut_cost_m3_override Clear-cut harvest cost override active when non-negative (USD17MER per m3) / 15 /
;

*' primf_harvest_aug26: regional parameters for the primary-forest clear-cut wood
*' harvest pathway. Indexed by i so further regions can be added by editing only
*' the CSV below -- no code change needed. Values only take effect for i in
*' policy_countries35_clearcut; elsewhere v35_clearcut_primforest is fixed to 0
*' in presolve.gms regardless of what is set here.
table f35_clearcut_regional_params(i,clearcut_par35) Clear-cut wood harvest regional parameters
$ondelim
$include "./modules/35_natveg/primf_harvest_aug26/input/f35_clearcut_regional_params.csv"
$offdelim
;

p35_clearcut_wood_recovery_share(i) = f35_clearcut_regional_params(i,"recovery_share");
p35_clearcut_harvest_cost_m3(i)     = f35_clearcut_regional_params(i,"harvest_cost_m3");
...
* primf_harvest_aug26 keeps its own copy of this table (includes an npiweak
* column not present in the shared modules/35_natveg/input/ file) so that
* pot_forest_may24 stays untouched and unaffected by this realization's data.
...
$ondelim
$include "./modules/35_natveg/primf_harvest_aug26/input/npi_ndc_ad_aolc_pol.cs3"
$offdelim
;
