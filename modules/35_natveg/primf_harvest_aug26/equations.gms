...
*' primf_harvest_aug26: clear-cut harvest cost is charged on the biomass actually
*' recovered (v35_clearcut_primforest scaled by the regional recovery share and
*' growing stock), not on the full felled area -- the $/m3 extraction-cost figures
*' this is based on cover the cost of getting the salvaged wood to the mill, not
*' the unrecovered share that is burned or abandoned on site.
...
q35_cost_hvarea(i2)..
                 ...
                 + sum((ct,cell(i2,j2)),
                     v35_clearcut_primforest(j2) * p35_clearcut_wood_recovery_share(i2)
                     * im_growing_stock(ct,j2,"acx","primforest")) * p35_clearcut_harvest_cost(i2)
                    ;
...
*' primf_harvest_aug26: primary forest that is directly deforested (cleared for
*' conversion to agricultural or other non-forest land, via v35_clearcut_primforest)
*' also contributes to wood production, discounted by the regional recovery share
*' (p35_clearcut_wood_recovery_share) reflecting that clear-cut salvage recovers
*' less merchantable biomass than selective harvest.

q35_prod_primforest(j2)..
              sum(kforestry, vm_prod_natveg(j2,"primforest",kforestry))
              =e=
              ( v35_hvarea_primforest(j2)
              + v35_clearcut_primforest(j2) * sum(cell(i2,j2), p35_clearcut_wood_recovery_share(i2)) )
              * sum(ct, im_growing_stock(ct,j2,"acx","primforest")) / m_timestep_length_forestry;
...
*' primf_harvest_aug26: primary forest area lost in a period is split between the
*' selective-harvest pathway (v35_hvarea_primforest, reclassified as secdforest via
*' q35_secdforest_regeneration below) and the clear-cut pathway (v35_clearcut_primforest,
*' NOT reclassified -- it flows to agricultural/other land via vm_lu_transitions in
*' [10_land]). Any remaining reduction not claimed by either variable is unharvested
*' deforestation, exactly as in the base pot_forest_may24 realization.

q35_hvarea_primforest(j2)..
                v35_hvarea_primforest(j2) + v35_clearcut_primforest(j2)
                =l=
                v35_primforest_reduction(j2);
