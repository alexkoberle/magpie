...
* -----------------------------------------------------
* primf_harvest_aug26: region gate for clear-cut harvest
* -----------------------------------------------------
* Cell-level switch: 1 for clusters in a country listed in policy_countries35_clearcut,
* 0 elsewhere. Used in presolve.gms to fix v35_clearcut_primforest to 0 outside the
* enabled regions. NOTE: p35_clearcut_wood_recovery_share(i)/p35_clearcut_harvest_cost(i)
* are NOT finalized here -- the $/m3 -> $/tDM conversion needs im_vol_conv(i), which module
* 52 (a higher module number, running later in this same preloop pass) has not yet computed
* at this point. That conversion happens in presolve.gms instead.
p35_country_switch_clearcut(iso) = 0;
p35_country_switch_clearcut(policy_countries35_clearcut) = 1;
p35_cell_switch_clearcut(j) = sum(cell(i,j), sum(i_to_iso(i,iso), p35_country_switch_clearcut(iso)));
