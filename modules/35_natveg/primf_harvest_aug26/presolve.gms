...
if(s35_hvarea = 0,
 ...
 v35_clearcut_primforest.fx(j) = 0;
elseif s35_hvarea = 1,
...
* primf_harvest_aug26: exogenous clear-cut rate, same baseline/formula pattern as v35_hvarea_primforest above.
* Feasibility note: q35_hvarea_primforest requires v35_hvarea_primforest + v35_clearcut_primforest <= v35_primforest_reduction.
* Under this mode both are independently .fx'd from the same (vm_land.l - vm_land.lo) baseline, so if
* s35_hvarea_primforest + s35_hvarea_clearcut is set too high relative to what vm_land.lo(j,"primforest")
* structurally allows, the period becomes infeasible. Not a concern while s35_hvarea_clearcut stays at its
* default of 0 (i.e. under the default s35_hvarea=2 endogenous mode, this branch never executes at all).
 v35_clearcut_primforest.fx(j) = (vm_land.l(j,"primforest") - vm_land.lo(j,"primforest"))*s35_hvarea_clearcut*m_timestep_length_forestry;
);

* -----------------------------------------------------------
* primf_harvest_aug26: region gate and cost-unit conversion
* -----------------------------------------------------------
* Region gate: fix to 0 outside policy_countries35_clearcut, regardless of s35_hvarea mode
* (applied after the if/elseif above so it always has the final say for out-of-region cells).
v35_clearcut_primforest.fx(j)$(p35_cell_switch_clearcut(j) = 0) = 0;

* Sensitivity-sweep override: replaces the CSV-loaded p35_clearcut_harvest_cost_m3(i) for every
* region when s35_clearcut_cost_m3_override >= 0 (default -1 = inactive, use the CSV value as-is).
* Applied before the $/m3 -> $/tDM conversion below, so an override goes through the exact same
* density conversion a real calibrated value would.
p35_clearcut_harvest_cost_m3(i)$(s35_clearcut_cost_m3_override >= 0) = s35_clearcut_cost_m3_override;

* $/m3 -> $/tDM conversion for the clear-cut harvest cost. Done here rather than in preloop.gms
* because im_vol_conv(i) (module 52, a higher module number) is not yet populated with its real
* region-specific value during module 35's own preloop -- only after the full preloop pass across
* all modules has completed once, which is true by the time any module's presolve.gms first runs.
p35_clearcut_harvest_cost(i) = p35_clearcut_harvest_cost_m3(i) / im_vol_conv(i);
