...
* cap_apr26_ref specific
 q56_emis_co2eq(i,emis_source,pollutants)  Convert vm_emissions_reg to CO2eq (Tg CO2eq per yr)
 q56_emis_cap                              Global AFOLU emissions cap constraint (Tg CO2eq per yr)
;

positive variables
 vm_carbon_stock(j,land,c_pools,stockType)  Carbon stock (mio. tC)
 v56_slack_emis_cap                         Slack for cap infeasibility guard (Tg CO2eq per yr)
;
...
variables
 ...
 v56_emis_co2eq(i,emis_source,pollutants)   CO2-equivalent emissions (Tg CO2eq per yr)
;

*#################### R SECTION START (OUTPUT DECLARATIONS) ####################
parameters
 ov_carbon_stock(t,j,land,c_pools,stockType,type)   Carbon stock (mio. tC)
 ov56_slack_emis_cap(t,type)                        Slack for cap infeasibility guard (Tg CO2eq per yr)
 ...
 ov56_emis_co2eq(t,i,emis_source,pollutants,type)   CO2-equivalent emissions (Tg CO2eq per yr)
 ...
 oq56_emis_co2eq(t,i,emis_source,pollutants,type)   Convert vm_emissions_reg to CO2eq (Tg CO2eq per yr)
 oq56_emis_cap(t,type)                              Global AFOLU emissions cap constraint (Tg CO2eq per yr)
;
