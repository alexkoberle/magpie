...
* ---- cap_apr26_ref: CO2eq aggregation and cap constraint ----

*' Convert regional emissions to CO2-equivalent using AR5 GWP factors.
*' Sources excluded from the cap policy (p56_cap_mask = 0) contribute zero.

 q56_emis_co2eq(i2,emis_source,pollutants) ..
  v56_emis_co2eq(i2,emis_source,pollutants) =e=
    vm_emissions_reg(i2,emis_source,pollutants)
    * p56_gwp(pollutants)
    * p56_cap_mask(emis_source,pollutants);

*' Global AFOLU cap: total CO2eq from all in-scope sources must not exceed the
*' exogenous cap trajectory. v56_slack_emis_cap prevents hard infeasibility when
*' the biophysical floor exceeds the cap; its level is reported to the platform.

 q56_emis_cap ..
  sum((i2,emis_source,pollutants), v56_emis_co2eq(i2,emis_source,pollutants))
  =l=
